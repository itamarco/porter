  ...
  return (
    <div className="skeuo-card overflow-hidden">
      <button
        ...
        className={`w-full px-3 py-2 flex items-center justify-between outline-none transition-all ${
          loadingServices
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {selectedCount > 0 && (
            <span className="skeuo-badge px-1.5 py-0.5 text-[9px] font-bold text-skeuo-accent shadow-skeuo-inset-sm">
              {selectedCount}
            </span>
          )}
          {loadingServices && (
            <svg
              className="animate-spin h-3 w-3 text-skeuo-accent ml-1"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
      ...
      {isExpanded && (
        <div className="px-3 py-2 bg-skeuo-dark shadow-skeuo-inset border-t border-white/5">
          {serviceList.length === 0 ? (
            <p className="text-[10px] text-gray-400 italic">No services found</p>
          ) : (
