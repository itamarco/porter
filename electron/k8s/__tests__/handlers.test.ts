...
describe('setupK8sHandlers', () => {
  ...
  beforeEach(() => {
    ...
    (K8sClient as jest.Mock).mockImplementation(() => ({}));
  });
  ...
  it('should register get-clusters handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'get-clusters')?.[1];
  ...
  it('should register get-namespaces handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'get-namespaces')?.[1];
    expect(handler).toBeDefined();

    const result = await handler!(null, 'test-cluster');
    expect(result).toEqual(mockNamespaces);
    expect(getNamespaces).toHaveBeenCalledWith(expect.any(K8sClient), 'test-cluster');
  });
  ...
  it('should register get-services handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'get-services')?.[1];
    expect(handler).toBeDefined();

    const result = await handler!(null, 'test-cluster', 'default');
    expect(result).toEqual(mockServices);
    expect(getServices).toHaveBeenCalledWith(expect.any(K8sClient), 'test-cluster', 'default');
  });
  ...
  it('should register start-port-forward handler', async () => {
    ...
    (mockPortForwardManager.startPortForward as jest.Mock).mockResolvedValue('forward-id');

    setupK8sHandlers(mockIpcMain, mockPortForwardManager);

    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'start-port-forward')?.[1];
    expect(handler).toBeDefined();

    const result = await handler!(null, config);
    expect(result).toBe('forward-id');
    expect(mockPortForwardManager.startPortForward).toHaveBeenCalledWith(config);
  });
  ...
  it('should register stop-port-forward handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'stop-port-forward')?.[1];
    expect(handler).toBeDefined();

    const result = await handler!(null, 'forward-id');
    expect(result).toBe(true);
    expect(mockPortForwardManager.stopPortForward).toHaveBeenCalledWith('forward-id');
  });
  ...
  it('should register get-active-forwards handler', async () => {
    ...
    (mockPortForwardManager.getActiveForwards as jest.Mock).mockReturnValue(mockForwards);

    setupK8sHandlers(mockIpcMain, mockPortForwardManager);

    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'get-active-forwards')?.[1];
    expect(handler).toBeDefined();
  ...
  it('should register load-config handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'load-config')?.[1];
    expect(handler).toBeDefined();
  ...
  it('should register save-config handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'save-config')?.[1];
    expect(handler).toBeDefined();
  ...
  it('should register open-in-browser handler', async () => {
    ...
    const handler = mockIpcMain.handle.mock.calls.find((call) => call[0] === 'open-in-browser')?.[1];
    expect(handler).toBeDefined();
  ...
  it('should set up port forward update listener', () => {
    ...
    expect(mockPortForwardManager.on).toHaveBeenCalledWith('update', expect.any(Function));
    ...
    expect(mockBrowserWindow.webContents.send).toHaveBeenCalledWith('port-forward-update', status);
  });
